...
(function () {
  ...
  function refreshEditables() {
    ...
    lockPriceUnits();
    ensureDoctorThumbs();   // 사진 없는 의사 카드에도 사진 추가용 빈 썸네일 보장(bindImages 전에)
    bindImages();
  ...
  /* 사진이 없는 의사 카드(.doctor_card--noimg 또는 .doctor_thumb 자체가 없는 카드)도
     편집창에서 사진을 추가할 수 있도록, 빈 썸네일(placeholder)을 자동으로 끼워 넣는다.
     - bindImages()가 이 빈 썸네일에 클릭(이미지 변경)을 바인딩한다.
     - 저장 시 사진을 끝내 안 넣었으면 cleanupDoctors가 썸네일을 다시 제거하고 --noimg를 복원. */
  function ensureDoctorThumbs() {
    var root = q(SECTIONS.doctors);
    if (!root) return;
    qa(".doctor_card", root).forEach(function (card) {
      if (q(".doctor_thumb", card)) return;               // 이미 썸네일이 있으면 그대로
      var thumb = document.createElement("div");
      thumb.className = "doctor_thumb lp-img-empty";
      thumb.setAttribute("data-lp-thumb-added", "1");      // 편집기가 끼워넣은 빈 썸네일 표식
      thumb.setAttribute("data-imgph", tr("이미지"));
      var img = document.createElement("img");
      img.setAttribute("src", BLANK_IMG);
      img.setAttribute("alt", "");
      thumb.appendChild(img);
      card.insertBefore(thumb, card.firstChild);           // 카드 맨 위(.doctor_body 앞)
      card.classList.remove("doctor_card--noimg");          // 썸네일 영역이 보이도록
    });
  }
  ...
  function snapshot(sel, sectionName) {
    ...
    if (sectionName === "doctors") cleanupDoctors(clone);
    return clone.innerHTML;
  }

  /* 의료진 : 편집기가 끼워넣은 빈 썸네일 정리.
     - 사진을 실제로 추가하지 않은 카드(img가 BLANK_IMG) → 썸네일 제거 + doctor_card--noimg 복원
       (원래 사진 없던 카드 / 새로 추가했지만 사진 안 넣은 카드를 원본 마크업처럼 되돌림)
     - 사진을 추가한 카드 → 썸네일 유지 + doctor_card--noimg 제거 + 편집 표식 정리 */
  function cleanupDoctors(root) {
    qa(".doctor_card", root).forEach(function (card) {
      var thumb = q(".doctor_thumb", card);
      var img = thumb ? q("img", thumb) : null;
      var src = img ? img.getAttribute("src") : "";
      var hasImg = !!src && src !== BLANK_IMG;
      if (hasImg) {
        card.classList.remove("doctor_card--noimg");
        if (thumb) thumb.removeAttribute("data-lp-thumb-added");
      } else {
        if (thumb) thumb.remove();
        card.classList.add("doctor_card--noimg");
      }
    });
  }
